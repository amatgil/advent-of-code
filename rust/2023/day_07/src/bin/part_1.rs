...
fn eval_hand(hand: &Hand) -> HandValue {
    let mut matches: Vec<usize> = Vec::new();
    for c in hand.0.chars() { matches.push(hand.0.chars().filter(|&x| x == c).count()); }
    let max = *matches.iter().max().unwrap();
    if max == 5 || max == 4 || max == 1 {return HandValue::from_val(max);}
    else {
        if max == 3 {
            if matches.contains(&2) { return HandValue::FullHouse; }
            ...
            if number_of_twos == 1 { return HandValue::OnePair; }
            else { return HandValue::TwoPair; }
        }
    }
...
fn card_worth(label: char) -> usize {
    const CARD_WORTH: &str = "AKQJT98765432";
    CARD_WORTH.len() -  CARD_WORTH.find(label).unwrap()
}
...
pub fn test_evaluator() {
    ...
    let pairs = HashMap::from([
        ...
        (HandValue::TwoPair,      "KK677"), (HandValue::TwoPair,      "KTJJT"),
        (HandValue::ThreeOfAKind, "T55J5"), (HandValue::ThreeOfAKind, "QQQJA"),
    ]);
